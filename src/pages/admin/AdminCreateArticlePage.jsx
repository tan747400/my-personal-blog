...
export default function AdminCreateArticlePage() {
  ...
  const save = async (publish = false) => {
    try {
      ...
      // resolve status id (draft / publish)
      const draftId = statuses.find(
        (s) => s.status?.toLowerCase() === "draft"
      )?.id;
      const publishId = statuses.find(
        (s) => s.status?.toLowerCase() === "publish"
      )?.id;
      ...
      <main className="flex-1 p-8 bg-gray-50 overflow-auto">
        ...
        >
          ...
          <div>
            ...
            <Textarea
              placeholder="Short intro that shows under the title on the article page"
              rows={5}
              maxLength={240}
            ...
            <div className="text-[12px] text-stone-500 mt-1">
              This will appear under the main headline.
            </div>
          </div>
